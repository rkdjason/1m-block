...
std::unordered_set<std::string> blocked_sites;
...
bool is_blocked(const char *host){
    ...
    double elapsed = (double)(end - start) / CLOCKS_PER_SEC;
    if (result)
        printf("[!] Host %s blocked (Search time: %.5fs)\n", host, elapsed);
              ...
              struct nfq_data *nfa, void *data)
{
    ...
    if (ret >= 0) {
        ...
        const char* host_ptr = strstr(http_packet, "Host: ");
        if (host_ptr == nullptr)
            return nfq_set_verdict(qh, id, NF_ACCEPT, 0, NULL);
...
int main(int argc, char **argv)
{
    ...
    if (!load_blocked_sites(argv[1])) {
        fprintf(stderr, "[Error] Failed to load blocked sites list\n");
        return EXIT_FAILURE;
    }
